...
public:
  ...
  int Get(TAirport airport) const {
    return data[get_airport_id(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    ++data[get_airport_id(airport)];
  }
  ...
  Items GetItems() const {
    ...
    for (size_t idx = 0; idx < SIZE; ++idx) {
      items[idx] = make_pair(static_cast<TAirport>(idx), data[idx]);
    }
    return items;
  }
...
private:
  ...
  uint32_t get_airport_id(TAirport airport) const {
