...
void SearchServer::UpdateDocumentBase(istream &document_input) {
    ...
    lock_guard data_guard(data_m);
    swap(index, new_index);
}

void SearchServer::GetQueriesStream(
    ...
    for (string current_query; getline(query_input, current_query);) {
        ...
        {
            lock_guard data_guard(data_m);
            auto doc_size = index.GetDocumentsSize();
            docid_count.assign(doc_size, 0);
            docids.resize(doc_size);
...
void SearchServer::AddQueriesStream(
    ...
    async_tasks.push_back(async([this, &query_input, &search_results_output](){
        GetQueriesStream(query_input, search_results_output); })
        );
}
